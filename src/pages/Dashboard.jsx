...
import { clinicProfile, doctors, events, galleryItems } from '../data/dummyData';
...
export default function Dashboard() {
  ...
  const rotatingEvents = Array.from(
    { length: Math.min(previewCount, events.length) },
    (_, i) => events[(eventIdx + i) % events.length]
  );
  const rotatingGallery = Array.from(
    { length: Math.min(previewCount, galleryItems.length) },
    (_, i) => galleryItems[(galleryIdx + i) % galleryItems.length]
  );
